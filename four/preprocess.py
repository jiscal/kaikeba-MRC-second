...
class Preprocessor:
    def __init__(self, datasets_fp, max_length=384, stride=128):
        ...
        self.bert_embeddings_index = {}
        self.embedding_matrix = []
        self.bert_embedding_matrix=[]
        self.word_list = []
        self.build_charset()
    ...
    def get_Bert_dataset(self, ds_fp):
        cs,qs,cqs, be = [],[],[], []
        contxt_questxt=open("content_questxt.txt",'w+',encoding="utf-8")
        for _, c,q,cq, b, e in self.get_Bdata(ds_fp):
            cs.append(c)
            qs.append(q)
            cqs.append(cq)
            contxt_questxt.write('\n'+cq)
            be.append((b, e))
        contxt_questxt.close()
        # print(c for c in ccs)
        return map(np.array, (cs,qs,cqs, be))
    #读取整句的context, question，并将他们合并储存list中，然后用bert提取句向量(extract_features.py)。
    ...
    #加载Bert的Embedding
    def load_BertEmbeding(self,Bert_file_path):
        Bert_Embeding=json.load(open(Bert_file_path,'r',encoding="utf-8"))
        bert_embeddings_index={}
        bert_embedding_matrix=[]
        features=Bert_Embeding['features']
        for i,feature in enumerate(features):
            bert_embeddings_index[i]=feature['token']
            bert_embedding_matrix.append(feature['token'])
        return bert_embedding_matrix

if __name__ == '__main__':
    p = Preprocessor([
        './data/squad/train-v1.1.json',
        './data/squad/test.json',
    ...
    #主要采用直接利用bert来获取向量
    #先处理数据将content 和question提取出来用|||合并用bert获取向量
    #利用extract_features.py(更改大小为100)文件里获取向量
    c_seg_list, q_seg_list,c_qm,be = p.get_Bert_dataset('./data/squad/train-v1.1.json')
