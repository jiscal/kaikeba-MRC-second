...
if __name__ == '__main__':
    ...
    c_seg_list, q_seg_list, c_qm, be = ds.get_Bert_dataset('./data/squad/test.json')
    bert_embedding_matrix=ds.load_BertEmbeding("./test/Bert_embedding.json")
    bidaf = BiDAF(
        clen=ds.max_clen,
        qlen=ds.max_qlen,
        ...
        embedding_matrix = ds.bert_embedding_matrix,
        max_features=len(ds.charset),
        conv_layers = [[10,1],[10,2],[30,3]]
    )
    bidaf.build_model()
    bidaf.model.fit(
        [c_seg_list,q_seg_list],test_y,
