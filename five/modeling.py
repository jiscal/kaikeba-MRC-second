...
# 双流自注意力部分
 # h 没有加mask_emb，只经过嵌入层的嵌入
 #g 增加了mask_emb
# r 相对位置的嵌入
# attn_mask_h去掉了自我，用于Query Stream，
#attn_mask_g允许看见自己，用于Content Stream
def two_stream_rel_attn(h, g, r, mems, r_w_bias, r_r_bias, seg_mat, r_s_bias,
  ...
  with tf.variable_scope(scope, reuse=False):
    ...
    ##### h-stream
    #Content Stream计算部分，利用了attn_mask_h
  ...
  with tf.variable_scope(scope):
   ...
   #假设inp_k[128,8]
    ...
    # data mask: input mask & perm mask
      #input_mask 0是真正的tokens而1是padding
    ...
    if data_mask is not None:
      ...
      if attn_mask is None:
          #xlnet中attn_mask=data_mask（qlen, qlen+mlen, batch, 1）
    ...
    if attn_mask is not None:
     ...
     #（qlen, qlen+mlen, batch, 1）
      non_tgt_mask = tf.concat([tf.zeros([qlen, mlen], dtype=tf_float),
                                non_tgt_mask], axis=-1)
     #Token能attend to 自己，除后面的对角线外，non_tgt_mask等于attn_mask，
     # 而对角线的位置由1变成了0，从而让它可以attend自己。用于后面的Content Stream
    ...
    if inp_q is not None:
      ...
      with tf.variable_scope('mask_emb'):
        ...
        if target_mapping is not None:
            #tile(复制)成[tf.shape(target_mapping)[0],bsz,d_model]
          word_emb_q = tf.tile(mask_emb, [tf.shape(target_mapping)[0], bsz, 1])
        else:
          inp_q_ext = inp_q[:, :, None]
          # 嵌入由两部分组成mask_emb和word_emb_k组成
          word_emb_q = inp_q_ext * mask_emb + (1 - inp_q_ext) * word_emb_k
        #没有加mask_emb，只经过嵌入层的嵌入
        #output_h是word_emb_k的dropout的结果，shape也是[128,8, d_model]
    ...
    ##### Segment embedding
      #seg_embed是Segment embedding，XLNet是相对Segment编码：
      # 如果两个Token在同一个Segment则是0；否则是1。
      ...
      with tf.variable_scope('layer_{}'.format(i)):
        ...
        if inp_q is not None:
          output_h, output_g = two_stream_rel_attn(
              ...
              #能代表自我，用于Content Stream
              attn_mask_h=non_tgt_mask,
              #不能代表自我，用于Query Stream
